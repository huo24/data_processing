...
request_streampulse_results <- function(sites) {
...
streampulse_data_pipeline <- function(site_list, raw_data_dest){
  ...
  for(site in site_list){
    s_code <- paste("NC", site, sep = "_")
    current_site <- request_data(s_code)
    component <- glue("streampulse_stream_chemistry_{tolower(gsub(' ', '', site))}")
    rawfile <- glue("{rd}/{c}.csv", rd = raw_data_dest, c = component)
    site_d <- current_site$data
    write_csv(site_d, file = rawfile)
    print(site)
  }
}
